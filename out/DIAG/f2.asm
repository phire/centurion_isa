...
887d:    6d a2        mov [--SP], r2
887f:    55 80        mov AX, r4
8881:    b5 a2        mov [--SP], AX
8883:    60 00 20     mov r2, 0x0020 ; This is a testing that the 60 3 byte instruction is working.
...
L_888f:
888f:    c5 61        mov BL, [r3++]
8891:    e5 01        mov [--AX], BL
8893:    3f           rotate_left AX
...
L_88f9:
88f9:    2a           clear AL ; This is potentially increment, 
...
894f:    06           set_carry
8950:    02           flag2
8951:    22 60        clear AH ; Using 60 in the operand means the following branch will be skipped
...
L_89c1:
89c1:    22 b0        clear AH ; Is B0 a two byte instrution that we haven't encountered yet?
89c3:    23 b0        neg? AH
...
L_8a6c:
8a6c:    3e           rotate_right AX
8a6d:    18 01        b_gt L_8a70
8a6f:    00           HALT

L_8a70:
8a70:    3f           rotate_left AX
8a71:    14 01        b_z L_8a74
8a73:    00           HALT

L_8a74:
8a74:    3f           rotate_left AX
8a75:    16 01        b_lt L_8a78
8a77:    00           HALT

L_8a78:
8a78:    3e           rotate_right AX
...
8a7c:    3a           clear AX
8a7d:    3b           neg? AX
8a7e:    3c           shift_right AX
8a7f:    39           dec? AX
8a80:    3d           shift_left AX
8a81:    38           inc? AX
8a82:    38           inc? AX
8a83:    38           inc? AX
8a84:    38           inc? AX
...
L_8b95:
8b95:    45 69        mov r4_low, r3_high
8b97:    45 78        mov r4_high, r3_low
8b99:    44 c8        xor r4_high, r6_high
8b9b:    44 d9        xor r4_low, r6_low
...
8c02:    69 00 2c     mov [0x002c], r2
8c05:    18 01        b_gt L_8c08
8c07:    00           HALT

L_8c08:
8c08:    61 00 2c     mov r2, [0x002c]
...
8c3f:    92 00 12     mov AX, [[0x0012]]
8c42:    18 01        b_gt L_8c45
8c44:    00           HALT

L_8c45:
8c45:    d2 00 14     mov BX, [[0x0014]]
...
L_8c51:
8c51:    94 1c        mov AX, [pc+0x1c]
8c53:    18 01        b_gt L_8c56
8c55:    00           HALT

L_8c56:
8c56:    d4 19        mov BX, [pc+0x19]
...
L_8c61:
8c61:    93 0c        mov AX, unknown
8c63:    d3 0c        mov BX, unknown
8c65:    58           add BX, AX
...
L_8c73:
8c73:    60 00 20     mov r2, 0x0020
8c76:    9a           mov AX, single_byte[r2]
...
L_8c8e:
8c8e:    9a           mov AX, single_byte[r2]
8c8f:    14 01        b_z L_8c92
8c91:    00           HALT

L_8c92:
8c92:    95 42        mov AX, [r2++]
...
8c9f:    9a           mov AX, single_byte[r2]
8ca0:    65 28        mov r2, [BX++]
8ca2:    fe           mov single_byte[r6], BX
8ca3:    54 40        xor AX, r2
8ca5:    14 01        b_z L_8ca8
8ca7:    00           HALT

L_8ca8:
8ca8:    95 28        mov AX, [BX++]
8caa:    fc           mov single_byte[r4], BX
8cab:    65 29        mov r2, [BX++]
8cad:    02           flag2
...
L_8cb4:
8cb4:    99           mov AX, single_byte[BX]
8cb5:    65 2a        mov r2, [BX++]
8cb7:    fc           mov single_byte[r4], BX
...
L_8cbe:
8cbe:    60 00 10     mov r2, 0x0010
8cc1:    95 44        mov AX, [r2++]
...
L_8cca:
8cca:    95 45        mov AX, [r2++]
8ccc:    3b           neg? AX
8ccd:    14 01        b_z L_8cd0
8ccf:    00           HALT

L_8cd0:
8cd0:    95 45        mov AX, [r2++]
8cd2:    d5 45        mov BX, [r2++]
...
L_8cda:
8cda:    95 46        mov AX, [r2++]
8cdc:    d5 46        mov BX, [r2++]
...
L_8ce4:
8ce4:    60 00 14     mov r2, 0x0014
8ce7:    95 44        mov AX, [r2++]
8ce9:    d5 4c        mov BX, [r2++]
8ceb:    fe           mov single_byte[r6], BX
...
L_8cf2:
8cf2:    95 4c        mov AX, [r2++]
8cf4:    fc           mov single_byte[r4], BX
8cf5:    d5 4d        mov BX, [r2++]
8cf7:    02           flag2
...
L_8cfe:
8cfe:    95 44        mov AX, [r2++]
8d00:    d5 4e        mov BX, [r2++]
8d02:    fc           mov single_byte[r4], BX
...
L_8d36:
8d36:    3e           rotate_right AX
8d37:    09           ret

L_8d38:
8d38:    60 09 ef     mov r2, 0x09ef
...
8da5:    05           DI
8da6:    b5 05        mov [--AX], AX
8da8:    b7 05        mov unknown, AX
8daa:    b9           mov single_byte[BX], AX
8dab:    00           HALT
8dac:    00           HALT
8dad:    00           HALT
8dae:    00           HALT
8daf:    00           HALT
8db0:    00           HALT
8db1:    ff           mov single_byte[r7], BX
8db2:    ff           mov single_byte[r7], BX
8db3:    55 55        mov r2, r2
8db5:    aa           mov single_byte[r2], AL
8db6:    aa           mov single_byte[r2], AL
8db7:    00           HALT
8db8:    00           HALT
...
8e7f:    5e           mov r4, AX
8e80:    20 80        inc? AH
8e82:    60 00 20     mov r2, 0x0020

L_8e85:
8e85:    d0 01 00     mov BX, 0x0100

L_8e88:
8e88:    8b           mov AL, single_byte[r3]
8e89:    28           inc? AL
8e8a:    ab           mov single_byte[r3], AL
8e8b:    ac           mov single_byte[r4], AL
...
8e9d:    30 60        inc? AX
8e9f:    30 80        inc? AX
8ea1:    3f           rotate_left AX
8ea2:    15 e1        b_nz L_8e85
8ea4:    7f           unknown
8ea5:    45 09        mov r4_low, AH

L_8ea7:
8ea7:    2e 1c        ?? r12, r1
8ea9:    f8           mov single_byte[AX], BX
8eaa:    03           flag3
8eab:    00           HALT
8eac:    2e 0c        ?? r12, r0
8eae:    f9           mov single_byte[BX], BX
8eaf:    03           flag3
8eb0:    00           HALT
8eb1:    80 01        mov AL, 0x01

L_8eb3:
8eb3:    a5 a2        mov [--SP], AL
8eb5:    2a           clear AL
8eb6:    a5 a2        mov [--SP], AL
8eb8:    6d a2        mov [--SP], r2
8eba:    a5 a2        mov [--SP], AL
8ebc:    60 06 c2     mov r2, 0x06c2
...
L_8ecc:
8ecc:    2e 1c        ?? r12, r1
8ece:    f9           mov single_byte[BX], BX
8ecf:    03           flag3
8ed0:    00           HALT
8ed1:    2e 0c        ?? r12, r0
8ed3:    f8           mov single_byte[AX], BX
8ed4:    03           flag3
8ed5:    00           HALT
8ed6:    2a           clear AL
...
Fn_6d9:
8ed9:    2e 1c        ?? r12, r1
8edb:    f8           mov single_byte[AX], BX
8edc:    01           nop
8edd:    00           HALT
8ede:    2e 1c        ?? r12, r1

L_8ee0:
8ee0:    f9           mov single_byte[BX], BX
8ee1:    01           nop
8ee2:    20 2e        
...
8eeb:    01           nop
8eec:    60 2e 1c     mov r2, 0x2e1c
8eef:    fc           mov single_byte[r4], BX
8ef0:    01           nop
...
L_8f04:
8f04:    2e 0c        ?? r12, r0
8f06:    f8           mov single_byte[AX], BX
8f07:    01           nop
8f08:    00           HALT
8f09:    2e 0c        ?? r12, r0
8f0b:    f9           mov single_byte[BX], BX
8f0c:    01           nop
8f0d:    20 2e        
8f0f:    0c           unknown
8f10:    fa           mov single_byte[r2], BX
8f11:    01           nop
8f12:    40 2e        add r7_high, BH
8f14:    0c           unknown
8f15:    fb           mov single_byte[r3], BX
8f16:    01           nop
8f17:    60 2e 0c     mov r2, 0x2e0c
8f1a:    fc           mov single_byte[r4], BX
8f1b:    01           nop
8f1c:    80 2e        mov AL, 0x2e
8f1e:    0c           unknown
8f1f:    fd           mov single_byte[SP], BX
8f20:    01           nop
8f21:    a0 2e        unknown
8f23:    0c           unknown
8f24:    fe           mov single_byte[r6], BX
8f25:    01           nop
8f26:    c0 2e        mov BL, 0x2e
8f28:    0c           unknown
8f29:    ff           mov single_byte[r7], BX
8f2a:    01           nop
8f2b:    e0 09        unknown
...
8f6f:    8b           mov AL, single_byte[r3] ; Load Byte via base+index
...
8fcf:    2c           shift_right AL
8fd0:    2c           shift_right AL
