
L_ef00:
ef00:    1c 03        bs3 L_ef05
ef02:    71 ef c0     jump 0xefc0 L_efc0

L_ef05:
ef05:    80 01        mov A, 0x01
ef07:    1d 01        bs4 L_ef0a
ef09:    2a           clear A

L_ef0a:
ef0a:    a1 f1 4b     mov [0xf14b], A
ef0d:    90 01 00     mov BA, 0x0100
ef10:    b1 ff f0     mov [0xfff0], BA
ef13:    a1 ff f4     mov [0xfff4], A
ef16:    a1 ff f7     mov [0xfff7], A
ef19:    5f           mov SP, BA
ef1a:    90 e8 8f     mov BA, 0xe88f
ef1d:    b1 ff f2     mov [0xfff2], BA
ef20:    80 03        mov A, 0x03
ef22:    7b 06        call L_ef2a
ef24:    80 00        mov A, 0x00
ef26:    7b 02        call L_ef2a
ef28:    75 a0        jump A + 0xa0

L_ef2a:
ef2a:    a1 f1 48     mov [0xf148], A

L_ef2d:
ef2d:    81 f1 44     mov A, [0xf144]
ef30:    2c           shift_right A
ef31:    15 cd        bnz L_ef00
ef33:    10 f8        bc L_ef2d
ef35:    81 f1 45     mov A, [0xf145]
ef38:    c0 30        mov C, 0x30
ef3a:    42 31        and A, C
ef3c:    49           sub C, A
ef3d:    15 ee        bnz L_ef2d
ef3f:    09           ret

L_efc0:
efc0:    90 f0 b0     mov BA, 0xf0b0
efc3:    1d 03        bs4 L_efc8
efc5:    90 f0 a0     mov BA, 0xf0a0

L_efc8:
efc8:    55 0c        mov QU, BA
efca:    90 00 f0     mov BA, 0x00f0
efcd:    5f           mov SP, BA

L_efce:
efce:    7b 1f        call L_efef
efd0:    4d           mov C, A
efd1:    7b 1c        call L_efef
efd3:    5b           or DC, BA
efd4:    4e           unknown
efd5:    7b 18        call L_efef
efd7:    45 16        mov E, A
efd9:    7b 14        call L_efef
efdb:    4c           xor C, A

L_efdc:
efdc:    3f           unknown
efdd:    16 06        blt L_efe5
efdf:    7b 0e        call L_efef
efe1:    a5 61        mov [--EF], A
efe3:    73 f7        jump L_efdc

L_efe5:
efe5:    7b 08        call L_efef
efe7:    15 e2        bnz L_efcb
efe9:    45 99        mov L, L
efeb:    15 e1        bnz L_efce
efed:    75 60        jump A + 0x60

L_efef:
efef:    85 c8        mov A, [QU++]
eff1:    01           nop
eff2:    16 d7        blt L_efcb
eff4:    2c           shift_right A
eff5:    11 03        bnc L_effa
eff7:    a5 c8        mov [--QU], A
eff9:    03           fcn

L_effa:
effa:    2c           shift_right A
effb:    11 f2        bnc L_efef
effd:    8e           mov A, [QU]
effe:    48           add C, A
efff:    09           ret
